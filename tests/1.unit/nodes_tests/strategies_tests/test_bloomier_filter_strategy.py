...
import pytest
from exonware.xwnode.nodes.strategies.bloomier_filter import BloomierFilterStrategy
...
def empty_bloomier():
    ...
    return BloomierFilterStrategy(capacity=1000)
@pytest.fixture

def simple_bloomier():
    """Create Bloomier filter with data."""
    bloomier = BloomierFilterStrategy(capacity=1000)
...
class TestBloomierFilterStrategy:
    ...
    def test_size_operation(self, simple_bloomier):
        ...
        # Bloomier filter: self.size is the table size (int attribute),
        # use len() to get number of entries
        assert len(simple_bloomier) >= 0
