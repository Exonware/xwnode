"""
#exonware/xwnode/tests/1.unit/nodes_tests/strategies_tests/test_splay_tree_strategy.py

Comprehensive tests for SplayTreeStrategy.

Tests cover:
- Interface compliance (iNodeStrategy)
- Core operations (put, get, delete)
- Splaying operations (self-adjusting tree)
- Iterator protocol
- Performance characteristics (O(log n) amortized operations)
...
@pytest.fixture
def empty_tree():
    """Create empty splay tree strategy."""
    return SplayTreeStrategy()


@pytest.fixture
def simple_tree():
    """Create splay tree with simple data."""
    tree = SplayTreeStrategy()
...
class TestSplayTreeStrategyInterface:
    """Test SplayTreeStrategy implements iNodeStrategy interface correctly."""
    ...
    def test_get_operation(self, simple_tree):
        """Test get operation returns correct values (splays to root)."""
...
@pytest.mark.xwnode_unit
@pytest.mark.xwnode_node_strategy
class TestSplayTreeStrategySplaying:
    """Test splaying operations."""
    
    def test_splaying_on_access(self, simple_tree):
        """Test that accessed nodes are splayed to root."""
        # Accessing a key should splay it to root
        value = simple_tree.get('key2')
        assert value == 'value2'
        # After splaying, key2 should be at root (verify by accessing again)
        assert simple_tree.get('key2') == 'value2'
...
@pytest.mark.xwnode_unit
@pytest.mark.xwnode_node_strategy
class TestSplayTreeStrategyCore:
    """Test core SplayTreeStrategy functionality."""
...
@pytest.mark.xwnode_unit
@pytest.mark.xwnode_node_strategy
class TestSplayTreeStrategyEdgeCases:
