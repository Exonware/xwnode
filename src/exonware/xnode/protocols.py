#!/usr/bin/env python3
"""
Protocol interfaces for xNode Strategy Pattern.

This module defines the protocols that all node and edge strategies must implement.
"""
...
class NodeTrait(Enum):
    """Node capabilities and traits."""
    NONE = "none"
    LAZY_LOADING = "lazy_loading"
    OBJECT_POOLING = "object_pooling"
    CIRCULAR_REF_DETECTION = "circular_ref_detection"
...
class NodeMode(Enum):
    """Node strategy modes."""
    HASH_MAP = "hash_map"
    TREE_GRAPH_HYBRID = "tree_graph_hybrid"
...
class iNodeStrategy(ABC):
    ...
    compatibility with the xNode facade.
    """
...
class iEdgeStrategy(ABC):
    ...
    compatibility with the xNode graph operations, including advanced
...
class iNodeFacade(ABC):
    """
    Abstract interface for the xNode facade.
    
    This defines the public interface that xNode must implement.
    """
