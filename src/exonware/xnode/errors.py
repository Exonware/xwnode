"""
A+ Best Practice Error Handling for xNode Library.
...
class xNodeError(Exception):
    ...
    def add_context(self, **kwargs) -> 'xNodeError':
        """Add context information (chainable)."""
        self.context.update(kwargs)
        return self
    
    def suggest(self, suggestion: str) -> 'xNodeError':
...
class xNodePathError(xNodeError, KeyError):
    """
...
class xNodeTypeError(xNodeError, TypeError):
...
class xNodeValueError(xNodeError, ValueError):
...
class ErrorFactory:
    ...
    @staticmethod
    def path_not_found(path: str, segment: str = None, available_keys: List[str] = None) -> xNodePathError:
        """Fast path for common 'not found' errors."""
        return xNodePathError(path, segment, "not_found", available_keys=available_keys)
    
    @staticmethod
    def type_mismatch(path: str, segment: str, node_type: str) -> xNodePathError:
        """Fast path for type mismatch errors."""
        return xNodePathError(path, segment, "type_mismatch", node_type=node_type)
    
    @staticmethod
    def index_out_of_bounds(path: str, index: int, length: int) -> xNodePathError:
        """Fast path for index errors."""
        return xNodePathError(path, str(index), "out_of_bounds").add_context(
            index=index, length=length
        ).suggest(f"Valid range: 0-{length-1}")
    
    @staticmethod
    def operation_not_supported(operation: str, node_type: str, supported_ops: List[str] = None) -> xNodeTypeError:
        """Fast path for unsupported operation errors."""
        message = f"Operation '{operation}' not supported on {node_type}"
        error = xNodeTypeError(
            message,
...
class xNodeStrategyError(xNodeError):
    """Base exception for strategy-related errors."""
    pass


class xNodeUnsupportedCapabilityError(xNodeStrategyError):
...
class xNodePresetError(xNodeStrategyError):
    ...
    def __init__(self, preset_name: str, operation: str, reason: str = None):
        ...
        try:
            from .strategies.types import list_presets
...
class xNodeSecurityError(xNodeError):
    """Base exception for security-related errors."""
    pass


class xNodeLimitError(xNodeSecurityError):
...
class xNodePathSecurityError(xNodeSecurityError):
...
xNodeValidationError = xNodeValueError
xNodeSerializationError = xNodeError
xNodePerformanceError = xNodeError  
xNodeConfigurationError = xNodeError

# Strategy errors (maintained for compatibility)
xNodeIllegalMigrationError = xNodeStrategyError
xNodeStrategyNotFoundError = xNodeStrategyError
xNodeStrategyInitializationError = xNodeStrategyError
xNodeConcurrencyError = xNodeStrategyError
...
__all__ = [
    ...
    # Strategy errors
    'xNodeStrategyError',
    'xNodeUnsupportedCapabilityError',
    'xNodePresetError',
    ...
    'xNodeValidationError',
    'xNodeSerializationError', 
    'xNodePerformanceError',
    'xNodeConfigurationError',
    'xNodeIllegalMigrationError',
    'xNodeStrategyNotFoundError',
    'xNodeStrategyInitializationError',
    'xNodeConcurrencyError'
]
